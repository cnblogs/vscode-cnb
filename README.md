...
- [功能概览](#功能概览)
	- [登录 / 授权](#登录-%2F-授权)
	- [将本地 markdown 发布到博客园](#将本地-markdown-发布到博客园)
	- [博客园博文列表](#博客园博文列表)
	- [搜索博文](#搜索博文)
...
也通过 VSCode 的 `Command Palette`（唤起 `Command Palette` 快捷键，windows：<kbd>ctrl</kbd>+<kbd>shift</kbd>+<kbd>p</kbd>，macOS：<kbd>cmd</kbd>+<kbd>shift</kbd>+<kbd>p</kbd>）调用 `Cnblogs: 上传到博客园`命令，将当前正在编辑的 markdown 上传到博客园。
...
一个本地文件可以关联到已有的博客园博文，VSCode打开本地文件，在右键菜单中点击`关联博客园博文`。

<kbd><img src="https://img2023.cnblogs.com/blog/35695/202311/35695-20231129180507395-887515545.png" width="550"></kbd>
...
本扩展支持对闪存进行**查看**，**发布**，**评论**等操作，在侧边栏博客园视图中，展开 `闪存` 即可; 点击闪存标题栏中的加号图标可以发布新闪存。
...
也可以通过 VSCode 命令面板(<kbd>ctrl/cmd</kbd>+<kbd>p</kbd>唤起命令面板)调用发闪存命令。
...
> **博客备份说明**
> 
> 博客备份会将您的博客中的随笔，文章等数据导出到一个 SqLite 的数据库文件中，此操作为耗时的任务式操作，完成后您可以在 [博客后台](https://i.cnblogs.com/posts/export) 或 [本扩展](https://marketplace.visualstudio.com/items?itemName=cnblogs.vscode-cnb) 中下载备份文件(后缀名为 `.db.zip`)
...
您可以将任何问题或建议反馈至 [GitHub Issues](https://github.com/cnblogs/vscode-cnb/issues)，我们会尽力解决。这对改善扩展体验至关重要。
